...
function Rooms() {
  ...
  const fetchRooms = async () => {
    try {
      const response = await axios.get('https://azouguaghapi-tl53v6k9.b4a.run/api/rooms');
  ...
  const handleCreateRoom = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://azouguaghapi-tl53v6k9.b4a.run/api/rooms', {
        name: newRoomName,
        admin_id: userId
      });
      setNewRoomName('');
  ...
  const handleJoinRequest = async (roomId, adminId) => {
    try {
      await axios.post('https://azouguaghapi-tl53v6k9.b4a.run/api/rooms/join', {
        room_id: roomId,
        user_id: userId
      });
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const res = await axios.post('https://azouguaghapi-tl53v6k9.b4a.run/api/upload-avatar', formData);
