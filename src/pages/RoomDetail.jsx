...
function RoomDetail() {
  ...
  useEffect(() => {
    ...
    axios.get(`https://azouguaghapi-7t2v1uze.b4a.run/api/rooms`).then(res => {
      const room = res.data.find(r => r.id == id);
      if (room && room.admin_id == userId) {
        setIsAdmin(true);
        axios.get(`https://azouguaghapi-7t2v1uze.b4a.run/api/rooms/${id}/requests`).then(r => setRequests(r.data));
      }
    });
  ...
  const handleResponse = async (uId, status) => {
    await axios.post('https://azouguaghapi-7t2v1uze.b4a.run/api/rooms/requests/respond', { room_id: id, user_id: uId, status });
